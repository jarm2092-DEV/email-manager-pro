...
value's alphanumeric runs joined by `[\s\-/.]*`, and tests it against both the normalized text and
a copy with separators stripped — that second copy is what catches "the list has it glued, the
email hyphenates it". The regexes are memoized per value, since the same 468 rows are searched on
every open. The same treatment covers project codes written as `CSP0001`.

The guard against matching a number inside another number stays: the value may not sit next to an
alphanumeric character. Verified over the real list — every permit and every project code, written
four ways each (as stored, glued, spaced, hyphenated), matches its own row and nothing else.

The body is read up to 40 000 characters, not 8 000: in a long thread the permit number ends up
buried at the bottom, and scanning the text costs milliseconds.
